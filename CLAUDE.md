...
```bash
pnpm vitest run tests/center-it-plugin.test.ts
```
...
**Source**: `src/center-it-plugin.ts` — the entire plugin. `src/index.ts` re-exports the default export as the public API entry point.
...
**Options** (`src/center-it-plugin.ts:3-6`):
